...
void UMannequinAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	if (EquippedWeapon && EquippedWeapon->GetWeaponMesh() && MannequinCharacter->GetMesh())
	{
		...
		if (MannequinCharacter->IsLocallyControlled())
		{
			...
			const FRotator LookAtRotation = UKismetMathLibrary::FindLookAtRotation(RightHandTransform.GetLocation(), RightHandTransform.GetLocation() + (RightHandTransform.GetLocation() - MannequinCharacter->GetHitTarget()));
